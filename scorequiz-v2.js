...
(function(){
  var root, fs, ref$, sum, average, toQuestionAndUserInfo, uinfoForUser, avgForUser, exec, ttest_rel, slice$ = [].slice;
  root = typeof exports != 'undefined' && exports !== null ? exports : this;
  fs = require('fs');
  ref$ = require('prelude-ls'), sum = ref$.sum, average = ref$.average;
  toQuestionAndUserInfo = function(filename){
    var lines, question_info, question_indexes, username_idx, i$, ref$, len$, idx, column_text, row_text, cells, username, j$, len1$, column_idx, cell_text, isLower, isUpper, splitAnswers, user_info, reference_user_idx, allchecked_user_idx, question_answers, uinfo, listContains, scoreAnswers, question_idx, answers, reference_answers, all_answers;
    ...
    for (i$ = 0, len$ = (ref$ = slice$.call(lines, 1)).length; i$ < len$; ++i$) {
      ...
      if (username === 'reference') {
        ...
        for (j$ = 0, len1$ = cells.length; j$ < len1$; ++j$) {
          column_idx = j$;
          cell_text = cells[j$];
          if (cell_text.trim() === '' && question_indexes[column_idx]) {
  ...
  (function(){
    var ref$, question_info_1, user_info_1, question_info_2, user_info_2, question_info_exam1, user_info_exam1, user_info_exam2, conditions, scores_invideo, scores_quizcram, scores_invideo_exam, scores_quizcram_exam, scores_part1, scores_part2, scores_part1_exam, scores_part2_exam, username, condition, score_part1, score_part2, score_exam1, score_exam2, score_invideo, score_quizcram, score_exam_invideo, score_exam_quizcram, usernames0, res$, usernames1, invideo_part1, i$, len$, user, quizcram_part1, invideo_part2, quizcram_part2, invideo_all, quizcram_all, part1_all, part2_all, invideo_exam_part1, quizcram_exam_part1, invideo_exam_part2, quizcram_exam_part2, invideo_exam_all, quizcram_exam_all, part1_exam_all, part2_exam_all;
    ref$ = toQuestionAndUserInfo('quiz3v2p1.tsv'), question_info_1 = ref$[0], user_info_1 = ref$[1];
    ref$ = toQuestionAndUserInfo('quiz3v2p2.tsv'), question_info_2 = ref$[0], user_info_2 = ref$[1];
    ref$ = toQuestionAndUserInfo('quiz2v2p1.tsv'), question_info_exam1 = ref$[0], user_info_exam1 = ref$[1];
    ref$ = toQuestionAndUserInfo('quiz2v2p2.tsv'), question_info_exam1 = ref$[0], user_info_exam2 = ref$[1];
    ...
    part1_all = invideo_part1.concat(quizcram_part1);
    part2_all = invideo_part2.concat(quizcram_part2);
    ...
    part1_exam_all = invideo_exam_part1.concat(quizcram_exam_part1);
    part2_exam_all = invideo_exam_part2.concat(quizcram_exam_part2);
    ...
    return console.log(ttest_rel(part1_exam_all, part2_exam_all));
    function fn$(){
      switch (condition) {
      case 0:
        return score_part1;
